...
app.post("/s1/exercice1", (req, res) => {
  ...
  if (n1 === undefined || n2 === undefined) {
    return res.json([{ reponse: 'Le nombre est invalide' }]);
  }
...
app.post("/s1/exercice3", (req, res) => {
  ...
  if (isNaN(n1) || isNaN(n2)) {
    return res.status(200).json([{ reponse: 'Le nombre est invalide' }]);
  }
 ...
 if (!isValidNumber(n1) || !isValidNumber(n2)) {
  ...
  if (isValidNumber(n1)  == null || nisValidNumber(n2)  == null) {
    return res.json([{ reponse: "Le nombre est invalide" }]);
  }
  if (isValidNumber(n1)  == null) {
    return res.json([{ reponse: "Le nombre est invalide" }]);
  }

  if (isValidNumber(n2)  == null) {
    return res.json([{ reponse: "Le nombre est invalide" }]);
  }

  let a = multiplication(isValidNumber(n1) , isValidNumber(n2) );
  return res.json([{ reponse: a }]);
});
...
app.post("/s2/exercice5", (req, res) => {
  ...
  if (isNaN(annee)|| annee == null || annee == undefined) {
    return res.status(400).json({ reponse: "Données invalides." });
  }
...
app.post("/s2/exercice6", (req, res) => {
  ...
  if (isNaN(date.getTime()) || date == null || date.getTime() == undefined) {
    return res
      .status(400)
